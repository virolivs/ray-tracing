#include "revolved_bezier.h"
#include <cmath>
#include <limits>
#include <algorithm>
#include <cassert>

namespace Geometry {

constexpr double TWO_PI = 6.283185307179586476925286766559;
constexpr double EPS_N  = 1e-9; 

static inline bool aabb_hit(const Point& bmin, const Point& bmax, const Ray& r) {
    double tmin = 0.0;
    double tmax = std::numeric_limits<double>::infinity();

    for (int axis = 0; axis < 3; ++axis) {
        const double o = (axis == 0) ? r.origin.x : (axis == 1) ? r.origin.y : r.origin.z;
        const double d = (axis == 0) ? r.direction.x : (axis == 1) ? r.direction.y : r.direction.z;
        const double slab_min = (axis == 0) ? bmin.x : (axis == 1) ? bmin.y : bmin.z;
        const double slab_max = (axis == 0) ? bmax.x : (axis == 1) ? bmax.y : bmax.z;

        if (d == 0.0) {
            if (o < slab_min || o > slab_max) return false;
            continue;
        }

        const double invD = 1.0 / d;
        double t0 = (slab_min - o) * invD;
        double t1 = (slab_max - o) * invD;
        if (invD < 0.0) std::swap(t0, t1);

        tmin = std::max(tmin, t0);
        tmax = std::min(tmax, t1);
        if (tmax < tmin) return false;
    }
    return true;
}


RevolvedBezierSurface::RevolvedBezierSurface(
    const std::vector<Point>& control_points,
    int degree,
    const Material& material
) : Hittable(material), control_points(control_points), degree(degree) {
    assert(degree >= 0);
    assert(static_cast<int>(control_points.size()) == degree + 1);
}


Point RevolvedBezierSurface::evaluate_curve(double u) const {
    if (u < 0.0) u = 0.0;
    else if (u > 1.0) u = 1.0;
    std::vector<Point> tmp = control_points;
    for (int r = 1; r <= degree; ++r) {
        for (int i = 0; i <= degree - r; ++i) {
            tmp[i] = tmp[i] + (tmp[i + 1] - tmp[i]) * u;
        }
    }
    return tmp[0];
}


Point RevolvedBezierSurface::surface_point(double u, double theta) const {
    const Point p = evaluate_curve(u);
    const double r = p.x;
    return Point(r * std::cos(theta), p.y, r * std::sin(theta));
}


void RevolvedBezierSurface::generateTriangles(std::vector<Triangle>& out,
                                              int u_res,
                                              int theta_res) const {
    out.reserve(out.size() + 2 * u_res * theta_res);

    for (int i = 0; i < u_res; ++i) {
        const double u0 = static_cast<double>(i) / u_res;
        const double u1 = static_cast<double>(i + 1) / u_res;

        for (int j = 0; j < theta_res; ++j) {
            const double t0 = (static_cast<double>(j) / theta_res) * TWO_PI;
            const double t1 = (static_cast<double>(j + 1) / theta_res) * TWO_PI;

            const Point p00 = surface_point(u0, t0);
            const Point p10 = surface_point(u1, t0);
            const Point p01 = surface_point(u0, t1);
            const Point p11 = surface_point(u1, t1);

            {
                const Vector e1 = p10 - p00;
                const Vector e2 = p11 - p00;
                const Vector n  = cross(e1, e2);
                if (e1.norm() > EPS_N && e2.norm() > EPS_N && n.norm() > EPS_N) {
                    out.emplace_back(p00, p10, p11, material);
                }
            }

            {
                const Vector e1 = p11 - p00;
                const Vector e2 = p01 - p00;
                const Vector n  = cross(e1, e2);
                if (e1.norm() > EPS_N && e2.norm() > EPS_N && n.norm() > EPS_N) {
                    out.emplace_back(p00, p11, p01, material);
                }
            }
        }
    }
}


void RevolvedBezierSurface::build() const {
    if (!built) {
        assert(degree >= 0);
        assert(static_cast<int>(control_points.size()) == degree + 1);
        assert(u_resolution >= 1);
        assert(theta_resolution >= 3);

        cached_triangles.clear();
        generateTriangles(cached_triangles, this->u_resolution, this->theta_resolution);
        built = true;
    }
}


RT::Trace RevolvedBezierSurface::hit(const Ray& ray) const {

    double rmax = 0.0;
    double ymin = +std::numeric_limits<double>::infinity();
    double ymax = -std::numeric_limits<double>::infinity();

    for (const auto& cp : control_points) {
        rmax = std::max(rmax, std::abs(cp.x));
        ymin = std::min(ymin, cp.y);
        ymax = std::max(ymax, cp.y);
    }

    const Point bmin(-rmax, ymin, -rmax);
    const Point bmax( rmax, ymax,  rmax);

    if (!aabb_hit(bmin, bmax, ray))
        return RT::Trace{ false, 0.0, ray.origin, {}, {}, this, -1 };

    build();

    bool   hit_any   = false;
    double closest_t = std::numeric_limits<double>::max();
    Point  hit_pos{};
    Vector hit_n{};
    int    hit_idx = -1;

    for (std::size_t i = 0; i < cached_triangles.size(); ++i) {
        const Triangle& tri = cached_triangles[i];

        RT::Trace res = tri.hit(ray);
        if (res.hit && res.t < closest_t) {
            hit_any   = true;
            closest_t = res.t;
            hit_pos   = res.position;

            Vector n = res.normal;
            if (n.norm() <= EPS_N) {
                const Point& a = tri.v0;
                const Point& b = tri.v1;
                const Point& c = tri.v2;
                n = cross(b - a, c - a);
                if (n.norm() > EPS_N) n = n.normalized();
                else                  n = Vector(0, 1, 0); 
            }

            if (dot(n, ray.direction) > 0.0) n = -n;

            hit_n   = n;
            hit_idx = static_cast<int>(i);
        }
    }

    if (hit_any) {
        return RT::Trace{ true, closest_t, ray.origin, hit_pos, hit_n, this, hit_idx };
    }
    return RT::Trace{ false, 0.0, ray.origin, {}, {}, this, -1 };
}

}