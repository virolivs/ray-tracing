...
Vector ray_color(const Ray& ray, const SceneLights& lights, int depth = 0) {
    ...
    if (mat.opacity < 1.0) {
        ...
        if (k >= 0) {
            ...
            refractedColor = ray_color(refractedRay, lights, depth + 1);
        } else {
                  ...
                  const SceneLights& lights)
{
    ...
    for (int j = image_height - 1; j >= 0; --j)
    {
        for (int i = 0; i < image_width; ++i)
        {
            ...
            Vector pixel_color = ray_color(ray, lights, 0);  // chamada recursiva com profundidade 0
